 ...
 * Cuando el API de recover esté listo, quita `.skip` de cada test y corre `node ace test`.
 */

test.group('Password recovery', () => {
  test.skip('accepts a recover request even if the email does not exist', async ({ client }) => {
  ...
  test.skip('does not include a reset code in the recover response', async ({ client, assert }) => {
    const response = await client.post('/api/v1/auth/recover').json({
  ...
  test.skip('rejects an invalid recovery code', async ({ client }) => {
  ...
  test.skip('resets the password with a valid code and allows login', async ({ client }) => {
